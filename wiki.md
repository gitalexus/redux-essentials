...
**Redux state обновляется при помощи "reducer functions":**

- Reducers всегда вычисляют новое состояние немутабельно, копирую текущие значения state и модифицируя копии новыми данными
...
Для чего обычно используются Redux middleware:

- выполнение дополнительной логики при запуске action (например логирование)
...
- учит как получить доступ к другим реальным объектам, а не только плоскому объекту action, таким как function, promises, class instances, для их перехвата и последующей обработки.
...
Типичный паттерн записи Thunk функции в виде creator аналогичный подходу с creator применяемыми для селекторов:\

```js
// creator - пишется в виде функции принимающей только параметры и возвращающей Thunk function
const logAndAdd = (amount) => {
  // thunk function всегда получаете dispatch и getState после чего они доступны для использования внутри неё
  return (dispatch, getState) => {
    // функция обычно реализует асинхронные действия дожидается окончания и вызывает синхронный action с помощью dispatch
